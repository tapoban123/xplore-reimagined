...
from .services import get_questions, get_careers, generate_pdf, store_psychometrics_to_db
...
def get_careers_for_user(user: Annotated[Student, Depends(validate_token)],
                         qna: QuestionsWithAnswersRequestModel, db: db_dependency):
    careers_output = get_careers(qna.questions_with_answers)
    pdf = generate_pdf(user, careers_output)

    result = cloudinary.uploader.upload(file=pdf,
