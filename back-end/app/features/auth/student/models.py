...
def dob_not_fake(dob: datetime):
    if get_age_from_dob(datetime.now(timezone.utc), dob) <= 3:
        raise HTTPException(
            detail="Date of birth must be greater than 3 years.",
...
class SignUpUserModel(AuthBaseModel):
    ...
    date_of_birth: Annotated[datetime, AfterValidator(dob_not_fake)] = Field(
        default=None, description="Date of birth must be greater than 3 years."
    )
